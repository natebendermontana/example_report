...
investment_levels <- c("Executive", "Management", "Contributor")
...
fake_attributes_lookup <- tibble(
  ...
  investment_level = sample(investment_levels, length(unique(all_fake_ids)), replace = TRUE),
...
df_deidentified <- bind_rows(df_fake_activities, df_fake_placeholders) %>%
  ...
  select(salesforce_id, first_name, last_name, city, state_name, county_fips, org_group, org_status, investment_level, is_activity, everything())
...
```{r, echo=F}
for_plot <- df_deidentified %>%
  group_by(investment_level, salesforce_id) %>%
  summarize(engaged = any(is_activity), .groups = "drop") %>%
...
overall_plot_data <- for_plot %>%
  filter(investment_level == "Overall")

for_plot <- for_plot %>%
  filter(investment_level != "Overall") %>% 
  mutate(    investment_level = factor(
    investment_level,
    levels = c("Executive", "Management", "Contributor")
  ))

main_plot <- ggplot(for_plot, aes(x = investment_level, y = Percentage, 
                                  fill = Activity_Status)) +
  geom_bar(stat = "identity", alpha = .9) +
  scale_fill_manual(
    values = c("engaged" = "#004d65", "didnotengage" = "#e25b5b"),
...
overall_plot <- ggplot(overall_plot_data, aes(x = investment_level, y = Percentage, fill = Activity_Status)) +
  geom_bar(stat = "identity", alpha = .9) +
  scale_fill_manual(
    values = c("engaged" = "#004d65", "didnotengage" = "#e25b5b"),
    labels = c("engaged" = "Engaged", "didnotengage" = "Did Not Engage")
...
```{r, echo=F,  fig.align="center", out.width="100%"}
for_plot <- df_deidentified %>%
  mutate(investment_level = factor(investment_level, levels = c("Executive", "Management", "Contributor"))) %>% 
  ...
  summarize(salesforce_id = first(salesforce_id),
            ...
            investment_level = first(investment_level),
            org_group = first(org_group)) %>%
  group_by(org_group, investment_level) %>%
  summarize(count = n(), .groups = "drop")

ggplot(for_plot, aes(x = reorder(org_group, count, FUN="sum", decreasing=T), y = count, fill = investment_level)) +
...
reference_table <- overall_avg_table %>%
  ...
  data_color(
    columns = c(Value),
    colors = function(x) redgreen_gradient((x - overall_per_quarter_avg) / overall_per_quarter_avg)
  ) %>%
  fmt_number(
    columns = c(Value),
    decimals = 2
...
team_activity_plot <- function(df, team = NULL, member_status = NULL, invest_color_pal, global_max_engaged) {
  ...
  top_engagers_all <- filtered_data %>%
    group_by(salesforce_id, first_name, last_name, investment_level) %>%
    summarize(engaged = sum(is_activity), .groups = "drop") %>%
    ungroup() %>%
    arrange(desc(engaged)) %>%
    mutate(
      full_name = paste(first_name, last_name),
      axis_label_color = as.character(investment_level)
    )
  ...
  # Create the plot
  ggplot(top_engagers_all, aes(x = full_name, y = engaged, fill = investment_level)) +
...
for_table <- df_deidentified %>% 
  ...
  select(first_name, last_name, org_group, org_status, investment_level, is_activity, activity_type, everything())
