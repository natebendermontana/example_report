---
title: "Acme Corp Volunteer Program Analysis"
author: Nate Bender
date: "`r Sys.Date()`"
output:
  html_document:
...
ggplot(for_plot, aes(x = total_activities)) +
  ...
  labs(
    title = "Over half the company did not volunteer at all",
    x = "\nTotal Activities",
...
The **Sales department** is buoyed by high activity among some key folks like [random name, random name, random name...]. They're the second-largest department, yet produce `r round(sales_metric,1)` activities per employee — more than any other department. 

However, at `r round(marketing_metric,1)` activities per employee, the **Marketing department** is not far behind either — mostly thanks to lots of activity from [random name, random name, random name...]
...
ggplot(results_df, aes(x = permuted_diffs)) +
  geom_histogram(binwidth = 0.02, fill = "#004d65", boundary = 0) +
  geom_vline(xintercept = observed_diff, color = "#e25b5b", size = 1) +
  annotate("text", x = observed_diff+.25, y = max_y-5,
           label = paste0("Observed difference:\n", round(observed_diff, 2)), size=4, color = "#e25b5b", fontface = "bold") +
  labs(
...
all_plot <- ggplot(top_engagers_all, aes(x = reorder(full_name, engaged), y = engaged, fill = org_group)) +
  ...
  coord_flip() +
  scale_x_reordered() +
  scale_y_continuous(breaks = seq(0, max(top_engagers_all$engaged), by = 2)) +
...
final_plot <- ggplot(top_engagers, aes(x = full_name, y = engaged, fill = org_group)) +
  ...
  coord_flip() +  
  scale_x_reordered() +
  scale_y_continuous(breaks = seq(0, max(top_engagers$engaged), by = 2)) +
  scale_fill_manual(values = deidentified_color_pal)+
  labs(
...
activity_labels <- c(
  "community_cleanup" = "Community Cleanup",
  "mentorship_program" = "Mentorship Program",
  "nonprofit_fundraising" = "501c3 Fundraising",
  "stem_workshops" = "STEM Workshops",
  "food_bank_support" = "Food Banks",
  "disaster_relief" = "Disaster Relief",
  "animal_shelter" = "Animal Shelters",
  "environmental_advocacy" = "Env Advocacy"
)
...
create_radar_chart <- function(department_name, data) {
  df_filtered <- data %>%
    ...
    mutate(activity_type = recode(activity_type, !!!activity_labels))  # Apply shortened labels
  
  # Check if department has activity data
  if (nrow(df_filtered) == 0) {
    print(paste("No activity data for", department_name))
    return(NULL)  # Skip if no data
  }
  
  # Reshape data to wide format for ggradar
  df_wide <- df_filtered %>%
    pivot_wider(names_from = activity_type, values_from = activity_count, values_fill = 0)
  ...
  max_activity_count <- df_deidentified %>%
    filter(is_activity == TRUE) %>%
    group_by(org_group, activity_type) %>%
    summarise(activity_count = n(), .groups = 'drop') %>%
    summarise(max_value = max(activity_count, na.rm = TRUE)) %>%
    pull(max_value)
  max_activity_str <- as.character(max_activity_count)
  
  grid.max <- max_activity_count
  grid.min <- 0
  ...
  ggradar(df_radar, 
          font.radar = "sans",
          grid.min = 0,
          grid.mid = round(max_activity_count / 2),
          grid.max = max_activity_count,
          values.radar = c("", as.character(round(max_activity_count / 2)), max_activity_str),
          gridline.mid.colour = "#6495a9",
          gridline.max.colour = "#004d65",
          grid.label.size = 6,
          axis.label.offset = 1.2,
          gridline.label.offset = -0.3 * (grid.max - centre.y),
          axis.labels = str_wrap(colnames(df_radar)[-1]),
          plot.title = "",
          group.colours = color,
          axis.label.size = 4,
          group.point.size = 3,
...
radar_engineering <- create_radar_chart("Engineering", df_deidentified)
radar_accounting <- create_radar_chart("Accounting", df_deidentified)
radar_finance <- create_radar_chart("Finance", df_deidentified)
...
sf_use_s2(FALSE)
states_sf <- st_read(shapefile_path, quiet = TRUE) %>%
  rename(state_name = NAME) %>%
  st_make_valid() %>%  # Fix invalid geometries
  st_transform(5070) %>%  # Reproject to a projected CRS (meters)
  st_simplify(dTolerance = 10000) %>%  # Simplify in meters (10km)
  st_transform(4326)  # Convert back to WGS84 for mapping
...
This chart includes a **<span style="color:#00a2db">smoothed average trend line</span>**, calculated using nearby data points to create a locally weighted curve that highlights overall trends while reducing short-term fluctuations.
```{r}
...
year_breaks <- unique(year(for_timeline$activity_week))

timeline_plot <- ggplot(for_timeline, aes(x = activity_week, y = count)) +
  geom_line(size = 1, color = "#004d65") +
  geom_smooth(color = "#00a2db", linetype = "dashed", se = F)+
  geom_text(data = data.frame(activity_week = as.Date(paste0(year_breaks, "-07-01")), 
                              y = max(for_timeline$count) * 1.1, 
                              label = year_breaks), 
            aes(x = activity_week, y = y, label = label), 
  ...
  scale_x_date(
    breaks = seq(
      ...
      to = ceiling_date(max(for_timeline$activity_week), "year") - months(2),  # End before next year's Q1
      by = "2 months"
    ),
    labels = function(x) format(x, "%b")  # Display month abbreviations (Jan, Apr, Jul, Oct)
  ) +
  scale_y_continuous(breaks = seq(0, max(for_timeline$count, na.rm = TRUE), by = 2)) +
  labs(
...
### 📌 Pretty Table with Year Headers
...
<p align="center">
<img src="media/acme_corp_avg_spectrum.png" width="400">
</p>
...
# Individual Activity Breakdown {#team-activity}
...
team_activity_plot <- function(df, team = NULL, member_status = NULL, invest_color_pal, global_max_engaged) {
  ...
  ggplot(top_engagers_all, aes(x = full_name, y = engaged, fill = seniority_level)) +
    ...
    scale_y_continuous(
      ...
      breaks = seq(0, global_max_engaged, by = 2)
    ) +
...
- There is a case-insensitive overall search bar in the upper right. You can sort the entire table in ascending/descending order of each column, you can filter by specific column, and for certain columns that are lists of distinct categories (like Group), you can select one or more of the specific departments. This allows conditional filtering — for instance you could filter Investment Level to just "High", and also filter Group to both "Sales" and "Accounting" to show just the employees that meet all of those criteria.  
...
**[Note]**: for this demonstration I've stripped out this recommendations section as it doesn't make a whole lot of sense to write up a set of made-up recommendations for a bunch of made-up data!
But in the real versions of this general type of report this section has included a detailed list of recommendations for improving program buy-in and efficacy based on the analysis above and my knowledge of the client's business. 
